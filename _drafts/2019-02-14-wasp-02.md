...
{:.alert}
**WARNING**: As of the writing of this post (February 2019), it is my understanding that Xilinx Vivado *does not* support unconstrained arrays yet as defined in the VHDL-2008 standard. My code is tested with [ghdl][15] only. So it may it adjusting to run with other VHDL suits - if at all.
...
/home/code/vhdl/wasp/src/chips/sn74ahc573.vhdl:66:9:@751500ps:(assertion failure): Input signal changed during hold time!
./sim/chips/sn74ahc573_tb:error: assertion failed
./sim/chips/sn74ahc573_tb:error: simulation failed
{% endhighlight %}
...
As you can see, it's basically just the 74AHC573 with the OE pin tied to ground. Okay, but why write an extra model for the output module if it's basically the same as the chip model itself? One, it's more elegant since I only have to concern myself with the latch signal and can ignore the OE signal; this reduces the number of signals and complexity in the long run. Two, I can pretty much copy-paste the code for the address output module. Since I'm using a 2K x 8-bit RAM, I need an 11-bit module to indicate the current address on display. Here's the code for the 11-bit address output module:
...
<figure>
    ...
    <figcaption>The *Wasp* reacting to some input and control signals</figcaption>
</figure>
...
<figure>
    ...
    <figcaption>The *Wasp* so far</figcaption>
</figure>
