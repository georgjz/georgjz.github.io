...
As you can see, it's basically just the 74AHC573 with the <span style="text-decoration: overline">OE</span> pin tied to ground. Okay, but why write an extra model for the output module if it's basically the same as the chip model itself? One, it's more elegant since I only have to concern myself with the latch signal and can ignore the <span style="text-decoration: overline">OE</span> signal; this reduces the number of signals and complexity in the long run. Two, I can pretty much copy-paste the code for the address output module. Since I'm using a 2K x 8-bit RAM, I need an 11-bit module to indicate the current address on display. Here's the code for the 11-bit address output module:
